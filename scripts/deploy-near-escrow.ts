...
async function main(): Promise<void> {
  ...
  await ks.setKey(NETWORK_ID, MASTER_ID, KeyPair.fromString(MASTER_PK as any))

  if (ESCROW_PK) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR API requires any for KeyPair compatibility
    await ks.setKey(NETWORK_ID, ESCROW_ID, KeyPair.fromString(ESCROW_PK as any))
  }
  ...
  if (ESCROW_PK) {
    ...
    if (VIEW_METHOD) {
      ...
      const res = await escrow.viewFunction({
        contractId: ESCROW_ID,
        methodName: VIEW_METHOD,
        args: vargs
      })
  ...
  const createWithName = async (name: string): Promise<string> => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR API requires any for KeyPair compatibility
    const PUBLIC_KEY = KeyPair.fromString(MASTER_PK as any).getPublicKey()
    ...
    try {
      ...
      await master.createAccount(name, PUBLIC_KEY, AMOUNT)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR API requires any for KeyPair compatibility
      await ks.setKey(NETWORK_ID, name, KeyPair.fromString(MASTER_PK as any))
  ...
  if ((process.env.AUTO_GENERATE || "").toLowerCase() === "true") {
    ...
    if (attempts >= 20)
      throw new Error("AUTO_GENERATE failed to create a fresh subaccount after 20 attempts")
  }

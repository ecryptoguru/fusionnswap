...
async function checkBalances(): Promise<void> {
  ...
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR API AccountBalance type is incomplete
    console.log(`   Storage Used: ${(nearBalance as any).storageUsage || "N/A"} bytes`)
    ...
    try {
      ...
      console.log(
        `   Escrow Account: ${(parseInt(escrowBalance.available) / 1e24).toFixed(6)} NEAR`
      )
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR API Account type is incomplete
      const provider = (nearAccount as any).connection.provider

...
export class EnhancedCrossChainAgent {
  ...
  async initialize(): Promise<void> {
    ...
    await keyStore.setKey(
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR API requires any for KeyPair compatibility
      KeyPair.fromString(this.config.nearPrivateKey as any)
    )
  ...
  displayMetrics(): void {
    ...
    const successRate =
      this.metrics.nearTransactionsDetected > 0
        ? (
            (this.metrics.nearTransactionsProcessed / this.metrics.nearTransactionsDetected) *
            100
          ).toFixed(1)
        : "0"
  ...
  private async createEvmEscrowWithChainSignatures(
    ...
    try {
      ...
      const isMultiFill = Boolean(intent.metadata?.multiFill) || false
  ...
  private async processNearTransaction(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR transaction structure is dynamic
    ...
    try {
      ...
      if (tx.actions && tx.actions.length > 0) {
        for (const action of tx.actions) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR action types are dynamic
          if ((action as any).Transfer) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR action types are dynamic
            const transferAmount = (action as any).Transfer.deposit
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR action types are dynamic
      if (tx.actions && tx.actions[0] && (tx.actions[0] as any).FunctionCall) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR action types are dynamic
        const functionCall = (tx.actions[0] as any).FunctionCall
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR intent structure is dynamic

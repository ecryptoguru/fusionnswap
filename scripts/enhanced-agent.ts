...
import {ChainSignaturesService, createChainSignaturesService} from '../src/services/chain-signatures.js'
import {NearIntentsService, createNearIntentsService, ProcessedIntent} from '../src/services/near-intents.js'
...
export class EnhancedCrossChainAgent {
    ...
    private nearAccount: any // NEAR Account object requires dynamic method access
    ...
    constructor(config: AgentConfig, chainSignatures: ChainSignaturesService, nearIntents: NearIntentsService) {
        this.config = config
        ...
        if (process.env.PRIVATE_KEY) {
            this.evmWallet = new ethers.Wallet(process.env.PRIVATE_KEY, this.evmProvider)
        }
    }
    ...
    displayMetrics(): void {
        ...
        const successRate =
            this.metrics.nearTransactionsDetected > 0
                ? ((this.metrics.nearTransactionsProcessed / this.metrics.nearTransactionsDetected) * 100).toFixed(1)
                : '0'
        const ethSuccessRate =
            this.metrics.ethEscrowsCreated + this.metrics.ethEscrowsFailed > 0
                ? (
                      (this.metrics.ethEscrowsCreated /
                          (this.metrics.ethEscrowsCreated + this.metrics.ethEscrowsFailed)) *
                      100
                  ).toFixed(1)
                : '0'
        ...
        console.log(`🔍 Blocks Scanned: ${this.metrics.blocksScanned.toLocaleString()}`)
        console.log(`🎯 NEAR Transactions Detected: ${this.metrics.nearTransactionsDetected}`)
        console.log(`✅ NEAR Transactions Processed: ${this.metrics.nearTransactionsProcessed} (${successRate}%)`)
        ...
        console.log(`⛽ Total Gas Used: ${this.metrics.totalGasUsed.toLocaleString()} wei`)
        console.log(`⚡ Avg Processing Time: ${this.metrics.averageProcessingTime.toFixed(0)}ms`)
        console.log(`🔗 Last Successful TX: ${this.metrics.lastSuccessfulTransaction || 'None'}`)
     ...
     */
    private async handleProcessedIntent(processedIntent: ProcessedIntent): Promise<void> {
        ...
        console.log(`   ${processedIntent.intent.sourceChain} → ${processedIntent.intent.targetChain}`)
     ...
     */
    private async createEvmEscrowWithChainSignatures(processedIntent: ProcessedIntent): Promise<void> {
        ...
        try {
            ...
            const resolver = new ethers.Contract(this.config.resolverAddress, resolverAbi, this.evmProvider)
            ...
            const dstImmutables = createImmutables({
                ...
                timelocks: this.packEvmTimelocks(processedIntent.orderHash, Boolean(isMultiFill))
            })

            const srcCancellationTimestamp = BigInt(Math.floor(Date.now() / 1000) + 3600) // 1 hour

            // Create transaction request
            const txRequest = {
                to: this.config.resolverAddress,
                data: resolver.interface.encodeFunctionData('deployDst', [dstImmutables, srcCancellationTimestamp]),
            ...
            const signedTx = await this.chainSignatures.signEthereumTransaction(txRequest)
            ...
            console.error('❌ Failed to create EVM escrow with Chain Signatures:', error)
    ...
    private simulateEvmEscrowCreation(processedIntent: ProcessedIntent): void {
        ...
        console.log(`   Would deploy escrow for order: ${processedIntent.orderHash}`)
        console.log(`   Amount: ${processedIntent.intent.targetAmount} ETH`)
        console.log(`   Using Chain Signatures address: ${this.chainSignatures.getEthereumAddress()}`)
    }
    ...
    private startEvmEventMonitoring(): void {
        ...
        console.log(`   Resolver Address: ${this.config.resolverAddress}`)
        console.log(`   EVM RPC: ${this.config.evmRpcUrl}`)

        // Validate resolver address
        if (!ethers.isAddress(this.config.resolverAddress)) {
            console.error(`❌ Invalid resolver address: ${this.config.resolverAddress}`)
        ...
        this.evmProvider!.getBlockNumber()
            .then((blockNumber) => {
                console.log(`✅ EVM connection verified - Current block: ${blockNumber}`)
            })
            .catch((error) => {
        ...
        this.evmProvider!.on(filter, (log) => {
            console.log('🎯 EVM Event Detected!')
        ...
        // Also monitor all events from the resolver to debug
        this.evmProvider!.on(
            {
                address: this.config.resolverAddress
            },
            (log) => {
                console.log('📡 Any EVM event from resolver:')
        ...
        // Also poll for recent EVM events every 30 seconds as backup
        const evmPollInterval = setInterval(async () => {
            if (!this.isRunning) {
                clearInterval(evmPollInterval)

                return
            }

            await this.pollRecentEvmEvents()
        }, 30000)
    }

    /**
     * Poll for recent EVM events as backup to event listener
     */
    private async pollRecentEvmEvents(): Promise<void> {
        try {
            if (!this.evmProvider) return

            const currentBlock = await this.evmProvider.getBlockNumber()
            const fromBlock = Math.max(0, currentBlock - 20) // Check last 20 blocks for better coverage

            console.log(`🔍 Polling EVM blocks ${fromBlock} to ${currentBlock} for events`)
            ...
            const logs = await this.evmProvider.getLogs(filter)

            if (logs.length > 0) {
                console.log(`📡 Found ${logs.length} EVM events in recent blocks`)

                for (const log of logs) {
                    console.log(`🎯 Processing EVM event from block ${log.blockNumber}`)
                    // Create EVMLogEvent with required properties
                    const evmLogEvent = {
                        ...log,
                        logIndex: log.index || 0,
                        topics: [...log.topics] // Convert readonly array to mutable
                    } as unknown as EVMLogEvent
                    await this.handleEvmEvent(evmLogEvent)
                }
            }
        } catch (error) {
            console.debug('EVM polling error:', error)
        }
    }
    ...
    private startNearEventMonitoring(): void {
        ...
        // Poll for NEAR events every 30 seconds (reduced frequency to avoid rate limits)
        ...
        const metricsInterval = setInterval(() => {
            ...
            if (this.metrics.nearTransactionsDetected > 0 || this.metrics.blocksScanned > 1000) {
                this.displayMetrics()
            }
        }, 300000) // 5 minutes
    }
    ...
    private async handleEvmEvent(log: EVMLogEvent): Promise<void> {
        try {
            ...
            console.error('❌ Error handling EVM event:', error instanceof Error ? error.message : String(error))
        }
    }
    ...
    private async processEvmEvent(log: EVMLogEvent): Promise<void> {
        ...
        try {
            ...
            if (parsedLog && parsedLog.name === 'DstEscrowCreated') {
                ...
                const safetyDeposit = dstImmutables.safetyDeposit

                console.log(`📋 EVM Escrow Created:`)
                console.log(`   Order: ${orderHash}`)
                console.log(`   Amount: ${amount.toString()}`)
                console.log(`   Safety Deposit: ${safetyDeposit.toString()}`)
                console.log(`   Block: ${log.blockNumber}`)
                console.log(`   Transaction: ${log.transactionHash}`)
                ...
                if (this.config.autoLock) {
                    await this.createCorrespondingNearEscrow(orderHash, hashlock, amount, safetyDeposit, parsedLog)
                }
            }
        } catch (error) {
     ...
     */
    private detectMultiFillOrder(_hashlock: string): boolean {
        // For this system, we don't support multi-fill orders yet
        // All orders are single-fill with direct hashlocks
        return false
    }
    ...
    private async createCorrespondingNearEscrow(
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        parsedLog?: any // EVM log data structure varies
    ): Promise<void> {
        ...
        try {
            ...
            // ✅ PROPER ETH → NEAR CONVERSION
            // Current conversion rate: 1 ETH = 1 NEAR (1:1 ratio for testing)
            // ETH: 18 decimals (wei = 10^18)
            // NEAR: 24 decimals (yoctoNEAR = 10^24)

            // ✅ REAL CONVERSION: Handle actual 0.000000005 ETH transfer
            // 0.000000005 ETH = 5000000000 wei (10 digits)
            // 0.000000005 NEAR = 5000000000000000 yoctoNEAR (16 digits)
            // This is WELL within JavaScript's MAX_SAFE_INTEGER: 9,007,199,254,740,991 (16 digits)

            const nearAmountBigInt = amount * BigInt(1000000) // ETH wei * 10^6 = NEAR yoctoNEAR
            const nearAmountNumber = Number(nearAmountBigInt) // Convert to JavaScript number

            // Safety check: ensure it's within safe integer range
            const nearAmount = nearAmountNumber <= Number.MAX_SAFE_INTEGER ? nearAmountNumber : 1000000

            if (nearAmountNumber > Number.MAX_SAFE_INTEGER) {
                console.log(`⚠️  Amount ${nearAmountNumber} exceeds MAX_SAFE_INTEGER, using safe fallback`)
            } else {
                console.log(`✅ Amount ${nearAmountNumber} is within safe integer range`)
            }

            console.log(`💱 REAL ETH → NEAR CONVERSION:`)
            console.log(`   ETH Input: ${ethers.formatEther(amount)} ETH (${amount.toString()} wei)`)
            console.log(`   NEAR Output: ${nearAmount} yoctoNEAR (${ethers.formatUnits(nearAmount, 24)} NEAR)`)
            console.log(`   Conversion: ETH wei × 10^6 = NEAR yoctoNEAR`)
            console.log(`   Safe for JavaScript: ${nearAmount <= Number.MAX_SAFE_INTEGER ? '✅' : '❌'}`)

            console.log(`💱 Conversion Details:`)
            console.log(`   ETH Amount: ${amount.toString()} wei (${ethers.formatEther(amount)} ETH)`)
            console.log(`   NEAR Amount: ${nearAmount} yoctoNEAR (${ethers.formatUnits(nearAmount, 24)} NEAR)`)
            console.log(`   Conversion Rate: 1 ETH = 1 NEAR (1:1 ratio)`)
            ...
            if (parsedLog && parsedLog.args && parsedLog.args.dstImmutables) {
                ...
                if (immutables.maker) {
                    makerAddress = '0x' + BigInt(immutables.maker).toString(16).padStart(40, '0')
                }

                if (immutables.taker) {
                    takerAddress = '0x' + BigInt(immutables.taker).toString(16).padStart(40, '0')
                }

                if (immutables.token) {
                    tokenAddress = '0x' + BigInt(immutables.token).toString(16).padStart(40, '0')
                }
            }
            ...
            console.log(`   Using SAME hashlock for NEAR escrow (no conversion needed)`)

            // ✅ CRITICAL FIX: Extract actual timelocks from EVM event to match exactly
            const evmTimelocks = parsedLog.args.dstImmutables.timelocks
            const now = Math.floor(Date.now() / 1000) // Current timestamp in seconds

            // Decode packed EVM timelocks (same as transfer script)
            const DST_WITHDRAWAL = Number((evmTimelocks >> 128n) & 0xffffffffn)
            const DST_PUBLIC_WITHDRAWAL = Number((evmTimelocks >> 160n) & 0xffffffffn)
            const DST_CANCELLATION = Number((evmTimelocks >> 192n) & 0xffffffffn)

            console.log(`🔍 Extracted EVM timelocks:`)
            console.log(`   dst_withdrawal: ${DST_WITHDRAWAL}s`)
            console.log(`   dst_public_withdrawal: ${DST_PUBLIC_WITHDRAWAL}s`)
            console.log(`   dst_cancellation: ${DST_CANCELLATION}s`)

            const simpleTimelocks = {
                deployed_at: now,
                src_withdrawal: DST_WITHDRAWAL,
                src_public_withdrawal: DST_PUBLIC_WITHDRAWAL,
                src_cancellation: DST_CANCELLATION,
                src_public_cancellation: DST_CANCELLATION + 1800,
                dst_withdrawal: DST_WITHDRAWAL,
                dst_public_withdrawal: DST_PUBLIC_WITHDRAWAL,
                dst_cancellation: DST_CANCELLATION
            }

            // ✅ DEBUG: Log exact parameters being sent to NEAR contract
            const nearArgs = {
                ...
                amount: nearAmount, // u128 - already a safe number for current contract
                safety_deposit: Number(safetyDeposit), // u128 - convert back to number for current contract
                timelocks: simpleTimelocks, // Use object directly, not JSON string
                maker_near: this.config.nearAccountId,
                taker_near: this.config.nearAccountId
            }

            console.log('🔍 DEBUG: NEAR contract parameters:')
            console.log(`   Args: ${JSON.stringify(nearArgs, null, 2)}`)
            ...
            const result = await this.nearAccount.functionCall({
                ...
                attachedDeposit: safetyDeposit.toString() // Must match safety_deposit
            })
            ...
            if (error instanceof Error) {
                if (error.message.includes('exists')) {
                    console.log('ℹ️  NEAR escrow already exists - this is expected for duplicate attempts')
                } else if (error.message.includes('safety deposit mismatch')) {
                    console.log('⚠️  Safety deposit mismatch - check attached deposit amount')
                } else if (error.message.includes('invalid number')) {
                    console.log('⚠️  JSON deserialization error - check parameter types')
                } else {
    ...
    private async pollNearEvents(): Promise<void> {
        ...
        try {
            // ✅ FIXED: Check only 20 blocks to reduce RPC usage and avoid rate limits
            await this.checkRecentNearBlocks(20)
        } catch (error) {
            if (error instanceof Error && error.message.includes('Rate limits exceeded')) {
                console.log('⚠️ NEAR RPC rate limit hit, waiting 60 seconds...')
     ...
     */
    private packEvmTimelocks(orderHash?: string, isMultiFill: boolean = false): bigint {
     ...
     */
    private async processMultiFillOrder(orderHash: string, merkleRoot: string, totalAmount: bigint): Promise<void> {
        ...
        console.log(`✅ Multi-fill order registered: ${secrets.length} fills prepared`)
    }
     ...
     */
    private async checkRecentNearBlocks(blockCount: number): Promise<EscrowEvent[]> {
        ...
        try {
            ...
            if (this.nearIntents) {
                ...
                if (intentsStatus.listening) {
                    console.debug('NEAR Intents service is active - monitoring for NEAR→EVM transfers')
            ...
            if (this.nearAccount && this.config.nearEscrowId) {
                const withdrawalEvents = await this.checkNearWithdrawalEvents(blockCount)
                events.push(...withdrawalEvents)
            }

            console.debug(`Found ${events.length} NEAR events in last ${blockCount} blocks`)
        } catch (error) {
     ...
     */
    private async checkNearEscrowContract(blockCount: number): Promise<EscrowEvent[]> {
        const events: EscrowEvent[] = []
        ...
        try {
            ...
            // ✅ FIXED: Check fewer blocks to reduce RPC usage and avoid rate limits
            const blocksToCheck = Math.min(blockCount, 20)
            console.log(`🔍 Checking last ${blocksToCheck} NEAR blocks from height ${currentHeight}`)
            ...
            for (let i = 0; i < blocksToCheck; i++) {
                ...
                // Add small delay between block checks to avoid rate limits
            ...
            console.log(`Found ${events.length} NEAR events in last ${blocksToCheck} blocks`)
        } catch (error) {
            ...
            for (const chunk of block.chunks) {
                ...
                try {
                    ...
                    for (const tx of chunkDetails.transactions) {
                        // 🔧 DEBUG: Log all transactions for debugging
                        if (totalTxs <= 5) {
                            // Only log first few to avoid spam
                            console.log(`   🔍 TX: ${tx.hash}`)
                            console.log(`      From: ${tx.signer_id}`)
                            console.log(`      To: ${tx.receiver_id}`)
                            console.log(`      Expected escrow: ${this.config.nearEscrowId}`)
                            console.log(`      Match: ${tx.receiver_id === this.config.nearEscrowId}`)
                        }
                        ...
                        if (tx.receiver_id === this.config.nearEscrowId) {
                            ...
                            this.metrics.averageProcessingTime =
                                (this.metrics.averageProcessingTime * totalProcessed + processingTime) /
                                (totalProcessed + 1)
                        }
                    }
                } catch (chunkError) {
                    console.debug(`Error processing chunk in block ${blockHeight}:`, chunkError)
                }
            }

            if (totalTxs > 0) {
                console.log(`   📦 Block ${blockHeight}: ${totalTxs} total txs, ${escrowTxs} escrow txs`)
            }
        } catch (blockError) {
            ...
            if (actions && actions.length > 0) {
                for (const action of actions) {
                    if (action.Transfer) {
                        ...
                        console.log(`   💰 Amount: ${(parseInt(transferAmount) / 1e24).toFixed(6)} NEAR`)
                    ...
                    if (action.FunctionCall && action.FunctionCall.method_name.includes('create_src')) {
     ...
     */
    private async createEthEscrowForNearDeposit(nearTxHash: string, nearAmount: string): Promise<void> {
        ...
        try {
            ...
            const secret = ethers.keccak256(ethers.toUtf8Bytes(`secret-${nearTxHash}`))
            const hashlock = ethers.keccak256(secret)

            // Convert NEAR yoctoNEAR to ETH wei (proper decimal conversion)
            // NEAR uses 24 decimals, ETH uses 18 decimals
            // 1 NEAR = 10^24 yoctoNEAR, 1 ETH = 10^18 wei
            // To convert: yoctoNEAR / 10^6 = wei (since 24-18=6)
            const nearAmountBigInt = BigInt(nearAmount)
            const ethAmount = nearAmountBigInt / BigInt(10 ** 6) // Convert from 24 to 18 decimals

            console.log(`   🔄 Decimal conversion:`)
            console.log(`   NEAR: ${nearAmount} yoctoNEAR (24 decimals)`)
            console.log(`   ETH: ${ethAmount.toString()} wei (18 decimals)`)
     ...
     */
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    private async processNearTransaction(tx: {hash: string; actions?: any[]}, _txResult: unknown): Promise<void> {
        console.log(`🔍 Processing NEAR transaction: ${tx.hash}`)
        ...
        try {
            ...
            if (tx.actions && tx.actions.length > 0) {
                for (const action of tx.actions) {
                    if (action.Transfer) {
                        const transferAmount = action.Transfer.deposit
                        console.log(`💰 NEAR transfer detected: ${transferAmount} yoctoNEAR`)

                        // For direct transfers, create corresponding ETH escrow
                        const orderHash = ethers.keccak256(
                            ethers.concat([ethers.toUtf8Bytes('near-deposit-'), ethers.toUtf8Bytes(tx.hash)])
                        )

                        const secret = ethers.keccak256(ethers.toUtf8Bytes(`secret-${tx.hash}`))
                        const hashlock = ethers.keccak256(secret)
                        ...
                        await this.createCorrespondingEvmEscrow(orderHash, hashlock, BigInt(transferAmount))
            ...
            if (tx.actions && tx.actions[0] && tx.actions[0].FunctionCall) {
                ...
                if (methodName === 'create_src_simple' || methodName.includes('create_src')) {
                    console.log('🎯 NEAR → ETH escrow creation detected!')

                    try {
                        const args = JSON.parse(Buffer.from(functionCall.args, 'base64').toString())
     ...
     */
    private async checkNearWithdrawalEvents(blockCount: number): Promise<EscrowEvent[]> {
        const events: EscrowEvent[] = []

        try {
            console.debug(`Checking for NEAR withdrawal events in last ${blockCount} blocks...`)
    ...
    private async processNearToEvmIntent(intent: any): Promise<void> {
        ...
        await this.createCorrespondingEvmEscrow(
            ...
            intent.hashlock || '0x' + Buffer.from(intent.order_hash_hex.slice(2), 'hex').toString('hex'), // Use order hash as hashlock for simplicity
     ...
     */
    private async createCorrespondingEvmEscrow(orderHash: string, hashlock: string, amount: bigint): Promise<void> {
        ...
        try {
            ...
            const resolver = new ethers.Contract(process.env.RESOLVER_ADDRESS!, resolverAbi, this.evmWallet)
            ...
            const dstImmutables = {
                ...
                amount: amount,
                safetyDeposit: 0,
                timelocks: (BigInt(300) << 128n) | (BigInt(600) << 160n) | (BigInt(1200) << 192n) // Longer timelocks for destination
            }
            ...
            let gasLimit: bigint
            try {
                const estimatedGas = await resolver.deployDst.estimateGas(dstImmutables, srcCancellationTimestamp, {
                    value: amount
                })
                ...
                console.log(`   Gas Error: ${gasError instanceof Error ? gasError.message : String(gasError)}`)
            }

            // ✅ FIXED: Deploy EVM destination escrow with ETH (for NEAR → ETH transfers)
            console.log('🔧 Calling deployDst with parameters:')
            console.log(
                '   dstImmutables:',
                JSON.stringify(dstImmutables, (key, value) => (typeof value === 'bigint' ? value.toString() : value), 2)
            )
            ...
            console.log(`   📊 Total escrows created: ${this.metrics.ethEscrowsCreated}`)
     ...
     */
    private async handlePartialWithdrawal(orderHash: string, proofIndex: number): Promise<void> {
        ...
        console.log(`🔓 Processing partial withdrawal ${proofIndex + 1}/${order.fillCount}`)
    ...
    private async displayAgentStatus(): Promise<void> {
        ...
        console.log(`   initialized: ${this.nearIntents ? 'true' : 'false'}`)
        console.log(`   Listening: false`)
        console.log(`   Intents Contract: ${intentsStatus?.intentsContract || 'N/A'}`)
        console.log(`   Escrow Contract: ${this.config.nearEscrowId}`)
        ...
        console.log(`   NEAR Account: ${this.config.nearAccountId}`)
        console.log(`   NEAR Escrow: ${this.config.nearEscrowId}`)
        console.log(`   NEAR RPC: ${this.config.nearNodeUrl}`)
        console.log('========================\n')
    }
}
...
async function main(): Promise<void> {
    console.log('🌉 Enhanced Cross-Chain Agent with Chain Signatures & NEAR Intents')
    console.log('====================================================================\n')
    ...
    try {
        ...
        const config: AgentConfig = {
            ...
            filterDstChainId: process.env.FILTER_DST_CHAIN_ID ? parseInt(process.env.FILTER_DST_CHAIN_ID) : undefined
        }
        ...
        const agent = new EnhancedCrossChainAgent(config, chainSignatures, nearIntents)

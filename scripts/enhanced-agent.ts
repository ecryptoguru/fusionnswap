...
export class EnhancedCrossChainAgent {
    ...
    private async createEvmEscrowWithChainSignatures(processedIntent: ProcessedIntent): Promise<void> {
        ...
        try {
            ...
            const dstImmutables = createImmutables({
                ...
                timelocks: this.packEvmTimelocks(processedIntent.orderHash, isMultiFill)
            })
        ...
        try {
            ...
            if (parsedLog && parsedLog.name === 'DstEscrowCreated') {
                ...
                if (this.config.autoLock) {
                    await this.createCorrespondingNearEscrow(orderHash, hashlock, amount, parsedLog)
                }
            }
        } catch (error) {
    ...
    private async createCorrespondingNearEscrow(
        ...
        try {
            ...
            // ✅ FIXED: Use simple numeric timelocks instead of packed BigInt
            const simpleTimelocks = {
                src_withdrawal: 300, // 5 minutes
                src_public_withdrawal: 600, // 10 minutes
                src_cancellation: 1200, // 20 minutes
                dst_withdrawal: 180, // 3 minutes
                dst_public_withdrawal: 600, // 10 minutes
                dst_cancellation: 1200 // 20 minutes
            }
            ...
            const result = await this.nearAccount.functionCall({
                ...
                args: {
                    ...
                    safety_deposit: 0, // Use number instead of string
                    timelocks: JSON.stringify(simpleTimelocks), // Use simple numeric timelocks
            ...
            for (const chunk of block.chunks) {
                ...
                try {
                    ...
                    for (const tx of chunkDetails.transactions) {
                        ...
                        if (totalTxs <= 5) { // Only log first few to avoid spam

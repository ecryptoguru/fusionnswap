...
export class EnhancedCrossChainAgent {
    ...
    private async handleEvmEvent(log: EVMLogEvent): Promise<void> {
        try {
            ...
            console.error('❌ Error handling EVM event:', error)
        }
    }
    ...
    /**
     * Poll for NEAR events
     */
    private async pollNearEvents(): Promise<void> {
        if (!this.nearAccount) return

        try {
            // Query recent escrow events - implementation would check recent blocks
            // This is a placeholder for actual NEAR event polling logic
            await this.checkRecentNearBlocks(10)
        } catch (error) {
    ...
    /**
     * Check NEAR escrow contract for recent activity
     */
    private async checkNearEscrowContract(blockCount: number): Promise<EscrowEvent[]> {
        const events: EscrowEvent[] = []

        try {
            console.debug(`Checking NEAR escrow contract: ${this.config.nearEscrowId} for last ${blockCount} blocks`)

            if (!this.nearAccount) {
                return events
            }

            // ✅ IMPLEMENTED: Actual NEAR transaction monitoring
            ...
            // Check recent blocks for transactions to our escrow contract
                    ...
                    for (const chunk of block.chunks) {
                        ...
                        try {
                            ...
                            for (const tx of chunkDetails.transactions) {
                                // Check if transaction is to our escrow contract
                                if (tx.receiver_id === this.config.nearEscrowId) {
                                    console.log(`🔍 Found NEAR escrow transaction: ${tx.hash}`)
                                    ...
                                    if (txResult.status?.SuccessValue !== undefined) {
                                        // Successful transaction - check if it's an escrow creation
                                        await this.processNearTransaction(tx, txResult)
                                    }
                                }
                            }
                        } catch (chunkError) {
                            // Skip chunks that can't be retrieved
                            console.debug('Chunk retrieval error:', chunkError)
                        }
                    }
                } catch (blockError) {
                    console.debug(`Error retrieving block ${blockHeight}:`, blockError)
                }
            }
        } catch (error) {
     ...
     */
    private async processNearTransaction(tx: any, txResult: any): Promise<void> {
    ...
    private async createCorrespondingEvmEscrow(orderHash: string, hashlock: string, amount: bigint): Promise<void> {
        ...
        try {
            ...
            const resolverAbi = [
                'function deploySrc((bytes32 orderHash, bytes32 hashlock, uint256 maker, uint256 taker, uint256 token, uint256 amount, uint256 safetyDeposit, uint256 timelocks) srcImmutables, uint256 dstCancellationTimestamp) external payable',
            ...
            // Create source immutables for EVM escrow
            const currentTimestamp = Math.floor(Date.now() / 1000)
            const srcImmutables = {
                orderHash: orderHash,
                hashlock: hashlock,
                ...
                amount: amount,
                safetyDeposit: 0,
                timelocks: (BigInt(10) << 128n) | (BigInt(30) << 160n) | (BigInt(60) << 192n) // Test-friendly timelocks
            }

            const dstCancellationTimestamp = BigInt(currentTimestamp + 3600) // 1 hour

            // Deploy EVM source escrow with ETH
            const tx = await resolver.deploySrc(srcImmutables, dstCancellationTimestamp, {

...
function findBuildInfo(contractName: string): {
  input: unknown
  solcVersion: string
} {
  ...
  throw new Error(
    `Could not locate build-info for ${contractName} under dist/contracts/build-info`
  )
}
...
async function submitVerification(params: {
  ...
  if (params.constructorArgs)
    body.set("constructorArguements", params.constructorArgs) // note Etherscan misspelling
  ...
  if (json.status !== "1") {
    throw new Error(
      `Etherscan verify submission failed: ${json.message ?? ""} ${json.result ?? ""}`
    )
  }
...
async function main(): Promise<void> {
  ...
  let CONSTRUCTOR_ARGS_HEX = (process.env.CONSTRUCTOR_ARGS_HEX || "").replace(
    /^0x/,
    ""
  )
  ...
  if (!CONSTRUCTOR_ARGS_HEX && SIGNATURE && ARGS_JSON) {
    try {
      ...
      console.log(
        "[Verify] Computed CONSTRUCTOR_ARGS_HEX from SIGNATURE/ARGS_JSON"
      )
    } catch (e) {
      console.warn(

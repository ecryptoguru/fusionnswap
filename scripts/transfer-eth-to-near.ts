...
async function waitForNearEscrow(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR API account type is complex
  nearAccount: any,
  escrowId: string,
  orderHash: string,
  maxWaitMs = 45000
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- NEAR escrow return type is dynamic
): Promise<any> {
...
async function withdrawFromNear(
  ...
  try {
    ...
    console.log(`💰 Transferring ${ethers.formatUnits(nearTokenAmount, 24)} NEAR tokens...`)
    ...
    console.log(`  💰 ${ethers.formatUnits(nearTokenAmount, 24)} NEAR tokens transferred!`)
  } catch (error) {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await ks.setKey(NETWORK_ID, NEAR_ACCOUNT_ID, KeyPair.fromString(NEAR_PRIVATE_KEY as any))
    ...
    await withdrawFromNear(nearAccount, NEAR_ESCROW_ID, orderHash, secret, NEAR_ACCOUNT_ID)
